...
整條流程可在任何時間點中斷（Ctrl+C、關機、系統強制重啟），之後重新執行即可接續：
...
* 每一筆 LLM 結果**立刻** append 進 JSONL 並落地，不做批次緩衝 —— 斷在任何一刻，
  已完成的都留得住。
* 每支腳本啟動時讀取既有輸出，跳過已完成的 id，只補沒跑到的。
* 續跑前比對設定指紋（指令 SHA256、語料庫 hash、模型 digest、生成參數），
  **不符就拒絕續跑**並指出不符欄位 —— 否則同一個檔案會混入兩種設定的結果。
* `01_prepare_data.py`（切分）與 `03_build_corpus.py`（語料庫）若已完成會刻意跳過。
  它們的輸出 hash 被所有下游步驟驗證，重建一旦有任何差異，已完成條件的指紋檢查
  就會全部擋下。
